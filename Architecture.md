...
flowchart TD
    A[Users] -->C
    B[Users] -->D
    C[Instances] -->|Protocol Messages| E
    D[Instances]-->|Protocol Messages| E
    E[Public Key Directory] --> F{Transparency Log}
```

Instances will use [HTTP Signatures](https://swicg.github.io/activitypub-http-signature/)
...
For example: To prevent instances from censoring revocations, users will be able
...
This is your typical Fediverse instance software (Mastodon, etc.), provided they have been updated
...
A user wants to use End-to-End Encryption for ActivityPub direct messages to communicate with other users or share 
[Auxiliary Data](/Specification.md#auxiliary-data) with them via the PKD. The user already has an account at an instance
or creates one to use the PKD.

### Fediverse Operator (Fedi Operator)

The Fediverse Operator manages one or more instances. This operator is responsible for managing the users of their 
instances and for choosing a PKD instance for their instances.

### PKD Operator

The PKD Operator manages the daily operations of their PKD.
The PKD Operator adds the trusted instances to the PKD.

## Questions and Answers

### Why are Public Key Directories separate from instances?

As sketched in the diagrams above, each Public Key Directory will have a one-to-many relationship with 
instances.

The goal of this architecture decision is to achieve something close to what Taylor Hornby calls
[Userbase Consistency Verification](https://defuse.ca/triangle-of-secure-code-delivery.htm#:~:text=are%20successfully%20attacked.-,Userbase%20Consistency%2C%20Verification,-%3A).
...
malicious administrators. If the same entity controls the instance **AND** the Public Key Directory, they are
